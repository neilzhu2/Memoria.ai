...
const RecordingFlowTest: React.FC = () => {
  ...
  const handleStartRecording = async () => {
    try {
      await mockAudioStore.startRecording();
      setIsRecording(true);
  ...
  const handleStopRecording = async () => {
    try {
      ...
      const recording = await mockAudioStore.stopRecording();
  ...
  const handlePauseRecording = async () => {
    if (isRecording) {
      await mockAudioStore.pauseRecording();
    } else {
      await mockAudioStore.resumeRecording();
    }
  };
...
describe('Recording Flow Integration', () => {
  beforeEach(() => {
    ...
    mockAudioStore.recordingDuration = 0;
    mockAudioStore.lastError = null;
    ...
    mockAudioStore.startRecording.mockResolvedValue({
      id: 'test-recording',
      filePath: 'mock://recording.m4a',
      duration: 0,
    });

    mockAudioStore.stopRecording.mockResolvedValue({
      id: 'test-recording',
      filePath: 'mock://recording.m4a',
      duration: 30,
    });
  ...
  describe('Error Handling Integration', () => {
    it('handles recording start failure gracefully', async () => {
      mockAudioStore.startRecording.mockRejectedValueOnce(new Error('Recording failed'));
    ...
    it('handles recording stop failure gracefully', async () => {
      mockAudioStore.stopRecording.mockRejectedValueOnce(new Error('Stop failed'));
  ...
  describe('State Management Integration', () => {
    ...
    it('integrates properly with audio store permissions', async () => {
      ...
      mockAudioStore.permissions = { microphone: 'denied', mediaLibrary: 'granted' };
      mockAudioStore.requestMicrophonePermission.mockResolvedValue(false);
