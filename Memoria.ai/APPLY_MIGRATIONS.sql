-- ============================================================================
-- Memoria.ai Database Migrations
-- ============================================================================
-- INSTRUCTIONS:
-- 1. Go to: https://tnjwogrzvnzxdvlqmqsq.supabase.co/project/_/sql
-- 2. Copy this ENTIRE file
-- 3. Paste into the SQL Editor
-- 4. Click "Run" to execute all migrations
-- 5. Restart your Expo app after completion
-- ============================================================================

-- ============================================================================
-- MIGRATION 1: Create Topics System
-- Created: December 2, 2025
-- Description: Creates topic categories, recording topics, and user history
-- ============================================================================
...
-- ============================================
CREATE INDEX IF NOT EXISTS idx_topics_category ON recording_topics(category_id);
CREATE INDEX IF NOT EXISTS idx_topics_active ON recording_topics(is_active);
CREATE INDEX IF NOT EXISTS idx_user_history_user ON user_topic_history(user_id);
CREATE INDEX IF NOT EXISTS idx_user_history_shown_at ON user_topic_history(shown_at DESC);
CREATE INDEX IF NOT EXISTS idx_user_history_topic ON user_topic_history(topic_id);
...
DROP POLICY IF EXISTS "Anyone can view active categories" ON topic_categories;
...
DROP POLICY IF EXISTS "Anyone can view active topics" ON recording_topics;
...
DROP POLICY IF EXISTS "Users can view own topic history" ON user_topic_history;
CREATE POLICY "Users can view own topic history" ON user_topic_history
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own topic history" ON user_topic_history;
CREATE POLICY "Users can insert own topic history" ON user_topic_history
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own topic history" ON user_topic_history;
CREATE POLICY "Users can update own topic history" ON user_topic_history
  FOR UPDATE USING (auth.uid() = user_id);

-- ============================================
-- 6. Seed Data: Categories (10 categories)
-- ============================================
...
-- ============================================
-- 7. Sample Topics (50 initial topics across all categories)
-- ============================================
...
-- ============================================================================
-- MIGRATION 2: Add topic_id to memories table
-- Created: December 3, 2025
-- Description: Links memories with their recording topics
-- ============================================================================
...
-- ============================================================================
-- Next steps:
-- 1. Verify the migrations ran successfully (check for errors above)
-- 2. Restart your Expo app
-- 3. The home page category filtering should now work correctly
-- ============================================================================
